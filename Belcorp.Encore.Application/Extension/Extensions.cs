    ...
    public static class Extensions
    {
        ...
        public static List<Excel_DTO> ToExcel_DTO(this List<AccountsInformation_MongoWithAccountAndSponsor> list)
        {
            var result = list.Select(ai => new Excel_DTO()
            {
                ...
                PaidAsCurrentMonth = ai.PaidAsCurrentMonth_Des,
                PCV = ai.PCV,
                AccountPhone_1 = ai.Account.AccountPhones.Where(p => p.AccountPhoneID == 1).Select(p => p.PhoneNumber).FirstOrDefault(),
                AccountPhone_2 = ai.Account.AccountPhones.Where(p => p.AccountPhoneID == 2).Select(p => p.PhoneNumber).FirstOrDefault(),
                AccountPhone_3 = ai.Account.AccountPhones.Where(p => p.AccountPhoneID == 3).Select(p => p.PhoneNumber).FirstOrDefault(),
                AccountPhone_4 = ai.Account.AccountPhones.Where(p => p.AccountPhoneID == 4).Select(p => p.PhoneNumber).FirstOrDefault(),
                AccountPhone_5 = ai.Account.AccountPhones.Where(p => p.AccountPhoneID == 5).Select(p => p.PhoneNumber).FirstOrDefault(),
